  ...
  return (
    <section className="relative flex min-h-screen items-center bg-off-white">
      <div className="relative mx-auto w-full max-w-5xl px-6 py-28 short:pb-12 short:pt-28">
        ...
        <p className="texto-deck mt-6 short:mt-3 max-w-2xl">
          {t.lead}
        </p>

        <div className="mt-16 short:mt-8 grid gap-px overflow-hidden rounded-lg border border-linha bg-linha md:grid-cols-2">
          {t.itens.map((terr, i) => (
            <Link
              key={ANCORAS[i]}
              href={`/${lang}/o-que-movemos#${ANCORAS[i]}`}
              className="group block bg-off-white p-8 short:p-5 transition-colors duration-500 hover:bg-verde"
            >
