...
export default function ComoTrabalhamos({ lang = "pt" }: { lang?: Idioma }) {
  ...
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden bg-off-white">
      <div className="mx-auto w-full max-w-6xl px-6 py-28 short:pb-6 short:pt-28">
        <p className="mb-6 short:mb-2 text-center text-xs font-bold uppercase tracking-[0.18em] text-terracota">
          {t.eyebrow}
        </p>
        ...
        <div className="relative mt-12 short:mt-4 hidden md:block">
          ...
          />
          <div className="pointer-events-none absolute left-1/2 top-1/2 h-[620px] w-[620px] short:h-[400px] short:w-[400px] -translate-x-1/2 -translate-y-1/2">
            <OrbitaPrincipios
              ativo={ativo}
              aoAgir={(q) => setAgindo(q < 0 ? null : q)}
            />
          </div>
          <div className="relative grid min-h-[640px] short:min-h-[420px] grid-cols-2 content-between gap-x-[380px] py-6 short:py-2">
